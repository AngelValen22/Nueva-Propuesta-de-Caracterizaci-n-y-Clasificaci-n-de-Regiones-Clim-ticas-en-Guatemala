            ...
            for cant_latitud in latitud:
                if (cant_latitud >= 13.7149) & (cant_latitud <= 17.8188):
                    ...
                    for cant_tiempo in range(inicio,fin):
                        ...
                        variable_climatica = (ds.variables[ 'tmin' ][cant_tiempo, condicion_lat, condicion_lon ])
                        ...
                        nuevo_registro = {'fecha':tiempo_utc[cant_tiempo],'variable':'tmn','valor':exa,'lon':cant_longitud,'lat':cant_latitud,'fuente':'enacs'}
                        df_data:pd.DataFrame = pd.concat([df_data,pd.DataFrame([nuevo_registro])],ignore_index=True)

    df_data.to_csv( 'tmin_enacs_' + str(inicio) + '_' + str(fin) + '.csv',index=False)


nombre_archivo = '/home/angelvalenzuela/EPS_tesis_regiones_climaticas/datos_crudos_fuente/ENACS/tmin.nc'
ds = nc.Dataset(nombre_archivo)
